    ...
    <Headless.Listbox {...props} multiple={false}>
      <Headless.ListboxButton
        ...
        className={clsx([
          ...
          // Disabled state
          'data-disabled:opacity-50 data-disabled:before:bg-muted data-disabled:before:shadow-none',
        ])}
      >
        <Headless.ListboxSelectedOption
          as="span"
          options={options}
          placeholder={placeholder && <span className="block truncate text-muted-foreground">{placeholder}</span>}
          className={clsx([
            // Basic layout
            ...
            'border border-input group-data-active:border-input/80 group-data-hover:border-input/80',
            'bg-transparent dark:bg-input/30',
            'group-data-invalid:border-destructive group-data-hover:group-data-invalid:border-destructive',
            'group-data-disabled:border-input/50 group-data-disabled:opacity-100 dark:group-data-disabled:border-input dark:group-data-disabled:bg-muted/50',
          ])}
        />
        <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
          <svg
            className="size-5 stroke-muted-foreground group-data-disabled:stroke-muted-foreground/70 sm:size-4 forced-colors:stroke-[CanvasText]"
            viewBox="0 0 16 16"
            aria-hidden="true"
            fill="none"
          >
...
>) {
  let sharedClasses = clsx(
    ...
    '*:data-[slot=icon]:text-muted-foreground group-data-focus/option:*:data-[slot=icon]:text-accent-foreground',
        ...
        return (
          <div
            className={clsx(
              ...
              'outline-hidden data-focus:bg-accent data-focus:text-accent-foreground',
              // Forced colors mode
...
export function ListboxDescription({ className, children, ...props }: React.ComponentPropsWithoutRef<'span'>) {
  return (
    <span
      ...
      className={clsx(
        className,
        'flex flex-1 overflow-hidden text-muted-foreground group-data-focus/option:text-accent-foreground before:w-2 before:min-w-0 before:shrink'
      )}
    >

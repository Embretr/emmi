...
export function DescriptionTerm({ className, ...props }: React.ComponentPropsWithoutRef<'dt'>) {
  return (
    <dt
      ...
      className={clsx(
        className,
        'col-start-1 border-t border-border/60 pt-3 text-muted-foreground first:border-none sm:border-t sm:border-border/60 sm:py-3'
      )}
    />
  )
}
...
export function DescriptionDetails({ className, ...props }: React.ComponentPropsWithoutRef<'dd'>) {
  return (
    <dd
      ...
      className={clsx(
        className,
        'pt-1 pb-3 text-foreground sm:border-t sm:border-border/60 sm:py-3 sm:nth-2:border-none'
      )}
    />
  )
}

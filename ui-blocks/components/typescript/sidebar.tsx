...
export function SidebarHeader({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex flex-col border-b border-border/60 p-4 [&>[data-slot=section]+[data-slot=section]]:mt-2.5'
      )}
    />
  )
}
...
export function SidebarFooter({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex flex-col border-t border-border/60 p-4 [&>[data-slot=section]+[data-slot=section]]:mt-2.5'
      )}
    />
  )
}
...
export function SidebarDivider({ className, ...props }: React.ComponentPropsWithoutRef<'hr'>) {
  return <hr {...props} className={clsx(className, 'my-4 border-t border-border/60 lg:-mx-4')} />
}
    ...
    <span className={clsx(className, 'relative')}>
      {current && (
        <motion.span
          ...
          className="absolute inset-y-2 -left-4 w-0.5 rounded-full bg-primary"
        />
      )}
